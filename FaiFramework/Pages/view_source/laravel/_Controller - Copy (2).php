...
?>
    ...
    }public function list($request,$page,$id)
    {
		...
		$page = 'list';
       	if($request->Cari =='pdf'){
        	...
        	return view('<?php echo $fai->nama_function($page,$page['title']);?>.list_<?php echo $fai->nama_function($page,$page['title']);?>',compact('page',<?=$compact?>'<?php echo $fai->nama_function($page,$page['title']);?>'));
		}  
    ...
    public function datatable($request,$page,$id)
    {
		 ...
		 $where= " 1=1 ";
			if (isset($search['value'])) {
				...
				for($i=0;$i<count($array);$i++){
					...
					if(in_array('select',$extypearray) and isset($array[$i][3][0]))
        ...
        $query = "select count(*) from (".$this->sql.")  as count";
		...
		$response = array(
            "draw" => intval($draw),
            "iTotalRecords" => count($count1),
            "iTotalDisplayRecords" => count($count2),
            "aaData" => $list
        );
    ...
    public function hapus($request,$page,$id)
    {
        DB::beginTransaction();
        try{
            $idUser=Auth::user()->id;
...
<?php }?>
    ...
    public function ajax_sub_kategori_<?=$database_sub?>(Request $request,$id)
    {
    	...
    	<?php 
			...
			echo str_replace("'{-30,3}'","&#92;'{-30,3}&#92;'",CrudContent::$var($page,$fai,$h,$no,$suffix));
